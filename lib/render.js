...
var debug = require('debug')('omelette:render');
...
function render (filepath, locals, altLayout) {
  return function (req, res, next) {
    ...
    if (altLayout) { 
      get_layout = template(altLayout);
    } else {
      get_layout = template('views/layout.jade');
    }
    function onLayout (err) {
      if (err) { return next(err); }
      debug('have "views/layout.jade" template');
      layout = altLayout ? req.templates[altLayout] : req.templates['views/layout.jade'];
